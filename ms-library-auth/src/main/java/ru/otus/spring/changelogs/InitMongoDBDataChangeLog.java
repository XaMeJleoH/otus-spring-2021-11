...
public class InitMongoDBDataChangeLog {
    ...
    @ChangeSet(order = "005", id = "initDefaultUser", author = "MakhanovAA", runAlways = true)
    public void initDefaultUser(UserRepository repository) {
        BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
        repository.save(new User("admin", passwordEncoder.encode("password"), Role.ADMIN));
        repository.save(new User("user", passwordEncoder.encode("password"), Role.USER));
    }
}
